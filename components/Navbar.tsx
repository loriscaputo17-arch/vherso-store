  ...
  return (
    <>
      <style>{`
        ...
        .sidebar-overlay { position:fixed; inset:0; z-index:199; background:rgba(0,0,0,0); pointer-events:none; transition:background 0.4s, backdrop-filter 0.4s; }
        ...
        @media (max-width:768px) { .sidebar{display:none!important;} .sidebar-overlay{display:none!important;} .hbd{display:none!important;} .ns{display:none!important;} .hbm{display:flex!important;} nav{height:56px!important;} nav.s{height:50px!important;} .mm{display:flex!important;} }
        @media (min-width:769px) { .hbm{display:none!important;} .mm{display:none!important;} }
      `}</style>
      ...
      <div className={`sidebar ${menuOpen ? 'open' : ''}`}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'3rem', paddingBottom:'1.5rem', borderBottom:'1px solid rgba(0,0,0,0.06)' }}>
          ...
          <button onClick={() => setMenuOpen(false)} style={{ background:'none', border:'none', cursor:'pointer', color:'rgba(0,0,0,0.3)', fontSize:'1rem', width:'28px', height:'28px', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:"'CenturyGothic',sans-serif", transition:'color 0.2s' }}>✕</button>
        </div>
      ...
      <nav className={scrolled ? 's' : ''} style={{ position:'fixed', top:0, left:0, right:0, zIndex:100, height:'68px', background:'rgba(245,245,245,0.96)', backdropFilter:'blur(24px)', borderBottom:'1px solid rgba(0,0,0,0.05)', transition:'height 0.4s cubic-bezier(0.25,0.46,0.45,0.94), background 0.4s, border-color 0.4s' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', height:'100%', padding:'0 2rem', position:'relative' }}>

          <div style={{ flexShrink:0 }}>
            <div className="hbd"><Hamburger onClick={() => setMenuOpen(!menuOpen)} /></div>
            <div className="hbm"><Hamburger onClick={() => setMenuOpen(!menuOpen)} /></div>
          </div>

          <Link href="/" style={{ position:'absolute', left:'50%', transform:'translateX(-50%)', display:'flex', alignItems:'center', textDecoration:'none' }}>
            <img src="/logo.png" alt="VHERSO" style={{ height: scrolled ? '40px' : '60px', width:'auto', display:'block', transition:'height 0.4s' }} />
          </Link>
      ...
      {/* MOBILE MENU */}
      <div className="mm" style={{ position:'fixed', top:0, left:0, right:0, bottom:0, zIndex:98, background:'#f5f5f5', display:'none', flexDirection:'column', paddingTop:'80px', transform: menuOpen ? 'translateX(0)' : 'translateX(-100%)', transition:'transform 0.5s cubic-bezier(0.25,0.46,0.45,0.94)', overflowY:'auto' }}>
        <div style={{ display:'flex', justifyContent:'center', padding:'0 2rem 2rem', borderBottom:'1px solid rgba(0,0,0,0.06)', marginBottom:'2rem' }}>
